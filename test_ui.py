...
BUFFER_SIZE = 4096  # Buffer size for reading process output
...
class CommandRunner:
    def __init__(self):
        self.result = (
            ui.markdown()
        )  # Create a markdown instance for displaying the result
...
class Validator:
    ...
    def validate_api_settings(self, formatter):
    ...
    def validate_led_settings(self):
    ...
    def verify_settings(self, stepper, setting_key, success_message):
        if self.storage.get(setting_key):
            stepper.next()
            ui.notify(success_message, type="positive")

    def verify_setup_is_complete(self, stepper, formatter):
...
def index():
    ...
    with ui.stepper().props("vertical").style(
        ...
        # Step 2: Setup LED strip
        with ui.step("Setup LED strip"):
            ui.input(
                "LED Pin", placeholder="D18", on_change=validator.validate_led_settings
            ).bind_value_to(storage, "led_pin")

            ui.input(
                "LED Count", placeholder="30", on_change=validator.validate_led_settings
    ...
    # Toggle visibility for control card once setup stepper is complete
    with ui.card().bind_visibility_from(storage, "setup_complete").props(
        ...
        ui.button(
            ...
            color="red",
            on_click=lambda: asyncio.create_task(
                runner.run_command("sudo pkill -f src/main.py")
            ),
        ).style("margin: 0 auto;")
